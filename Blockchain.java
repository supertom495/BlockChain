...
public class Blockchain{
    ...
    public boolean containsHash(byte[] hash) {
        ...
        for (int i = 0; i < getLength(); i++){
            if(Arrays.equals(block.getPreviousHash(), hash)) return true;
            block = block.getPreviousBlock();
        }
        return false;
    }

    public Block getBlockWithHash(byte[] hash) {
        Block block = getHead();

        while(!(block == null || Arrays.equals(block.calculateHash(), hash))) block = block.getPreviousBlock();
